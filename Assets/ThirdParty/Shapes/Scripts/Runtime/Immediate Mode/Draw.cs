    ...
    public static partial class Draw
    {
        ...
        public static DrawCommand Command(Camera cam)
        {
            switch (UnityInfo.CurrentRp)
            {
                ...
                default:
                    throw new Exception(
                        $"Failed to set up draw command. Current render pipeline appears to not be installed: {UnityInfo.CurrentRp}");
            }
        }
            ...
            [OvldDefault(nameof(Thickness))] float thickness)
        {
                       ...
                       sourceMesh: ShapesMeshUtils.GetLineMesh(Draw.LineGeometry, endCaps, DetailLevel)))
            {
                ...
                mpbLine.alignment.Add((float)Draw
                    .LineGeometry); // this is redundant for 3D lines, but, that's okay, fixing that makes things messier
            ...
            [OvldDefault(nameof(Color))] Color color)
        {
            ...
            {
                using (new IMDrawer(mpbPolylineJoins, ShapesMaterialUtils.GetPolylineJoinsMat(joins)[Draw.BlendMode],
                           mesh, 1))
        ...
        static void DiscCore(bool hollow, bool sector, float radius, float thickness, DiscColors colors,
            float angleRadStart = 0f, float angleRadEnd = 0f, ArcEndCap arcEndCaps = ArcEndCap.None)
        {
            ...
            using (new IMDrawer(mpbDisc, ShapesMaterialUtils.GetDiscMaterial(hollow, sector)[Draw.BlendMode],
        ...
        {
            using (new IMDrawer(mpbRegularPolygon, ShapesMaterialUtils.matRegularPolygon[Draw.BlendMode],
                       ShapesMeshUtils.QuadMesh[0]))
            {
        ...
        {
            using (new IMDrawer(mpbTriangle, ShapesMaterialUtils.matTriangle[Draw.BlendMode],
        ...
        static void Sphere_Internal([OvldDefault(nameof(Radius))] float radius,
            [OvldDefault(nameof(Color))] Color color)
        {
            using (new IMDrawer(metaMpbSphere, ShapesMaterialUtils.matSphere[Draw.BlendMode],
        ...
        static void Cuboid_Internal(Vector3 size,
            [OvldDefault(nameof(Color))] Color color)
        {
            using (new IMDrawer(mpbCuboid, ShapesMaterialUtils.matCuboid[Draw.BlendMode],
                       ShapesMeshUtils.CuboidMesh[0]))
            {
            ...
            [OvldDefault(nameof(Color))] Color color)
        {
            ...
            using (new IMDrawer(mpbTorus, ShapesMaterialUtils.matTorus[Draw.BlendMode],
        ...
        [OvldGenCallTarget]
        static void TextRect_Internal([OvldDefault("null")] string content,
        ...
        static OnPreRenderTmpDelegate OnPreRenderTmp
        {
            get
            {
                ...
                {
                    MethodInfo method = typeof(TextMeshPro).GetMethod("OnPreRenderObject",
        ...
        static void ApplyTextValuesToInstance(TextMeshProShapes tmp, bool isRect, string content, TMP_FontAsset font,
        ...
        static void CustomMesh_Internal(Mesh mesh, Material mat,
            MaterialPropertyBlock mpb /*, DrawCallProperties properties*/)
        {
            if (mesh == null)
                throw new NullReferenceException("null mesh passed into Draw.Mesh");
            using (IMDrawer drawer = new IMDrawer(mpbCustomMesh, mat, mesh, drawType: IMDrawer.DrawType.Custom,
                       allowInstancing: false))
            {
                // will draw on dispose
        ...
        static void Texture_Internal(Texture texture, Rect rect, Rect uvs, [OvldDefault(nameof(Color))] Color color)
        {
            ...
            if (mpbTexture.texture != null && mpbTexture.texture != texture)
            {
                DrawCommand.CurrentWritingCommandBuffer.drawCalls
                    .Add(mpbTexture.ExtractDrawCall()); // finalize previous buffer
            }
