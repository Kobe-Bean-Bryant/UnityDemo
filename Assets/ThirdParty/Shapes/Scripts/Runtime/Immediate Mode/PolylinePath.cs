...
namespace Shapes
{
    public class PolylinePath : PointPath<PolylinePoint>
    {
        ...
        [MethodImpl(INLINE)]
        public void BezierTo(Vector3 startTangent, Vector3 endTangent, Vector3 end) => BezierTo(startTangent,
            endTangent, end, ShapesConfig.Instance.polylineDefaultPointsPerTurn);
        ...
        [MethodImpl(INLINE)]
        public void BezierTo(Vector3 startTangent, Vector3 endTangent, PolylinePoint end) => BezierTo(startTangent,
            endTangent, end, ShapesConfig.Instance.polylineDefaultPointsPerTurn);
        ...
        public void BezierTo(Vector3 startTangent, Vector3 endTangent, PolylinePoint end, int pointCount)
        {
            if (CheckCanAddContinuePoint()) return;
            PolylinePoint
                ppB = PolylinePoint.Lerp(LastPoint, end, 1f / 3f); // blend all other properties, assume thirds
        ...
        {
            if (CheckCanAddContinuePoint()) return;
            int pointCount = CalcBezierPointCount(LastPoint.point, startTangent.point, endTangent.point, end.point,
                pointsPerTurn);
            BezierTo(startTangent, endTangent, end, pointCount);
        }
        ...
        [MethodImpl(INLINE)]
        public void ArcTo(Vector3 corner, Vector3 next, float radius) => AddArcPoints(corner, next, radius,
            useDensity: true, 0, ShapesConfig.Instance.polylineDefaultPointsPerTurn);
        ...
        [MethodImpl(INLINE)]
        public void ArcTo(Vector3 corner, PolylinePoint next, float radius) => AddArcPoints(corner, next, radius,
            useDensity: true, 0, ShapesConfig.Instance.polylineDefaultPointsPerTurn);
        ...
        void AddArcPoints(Vector3 corner, Vector3 next, float radius, bool useDensity, int targetPointCount,
            float pointsPerTurn)
        {
            if (CheckCanAddContinuePoint()) return;
        ...
        void AddArcPoints(Vector3 corner, PolylinePoint next, float radius, bool useDensity, int targetPointCount,
            float pointsPerTurn)
        {
            if (CheckCanAddContinuePoint()) return;
        ...
        [System.Obsolete(
            "This function no longer exists - either use the overload without a color, where the color will match the previous point, or the one with a PolylinePoint endpoint, where the color will blend between previous point and the target point",
            true)]
        public void ArcTo(Vector3 corner, Vector3 next, float radius, Color color) => AddArcPoints(corner, next, radius,
            useDensity: true, 0, ShapesConfig.Instance.polylineDefaultPointsPerTurn);
