...
namespace Shapes
{
    public class PolygonPath : PointPath<Vector2>
    {
        ...
        public void BezierTo(Vector2 startTangent, Vector2 endTangent, Vector2 end, float pointsPerTurn)
        {
            ...
            float curveSumDeg =
                ShapesMath.GetApproximateAngularCurveSumDegrees(LastPoint, startTangent, endTangent, end, sampleCount);
            float angSpanTurns = curveSumDeg / 360f;
            int pointCount = Mathf.Max(2,
        ...
        void AddArcPoints(Vector2 corner, Vector2 next, float radius, bool useDensity, int targetPointCount,
            float pointsPerTurn)
        {
            if (radius <= 0.0001f)
            {
