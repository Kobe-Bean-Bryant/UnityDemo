...
namespace Shapes
{
    public static class ShapesMath
    {
        ...
        public static Vector4 AtLeast0(Vector4 v) =>
            new Vector4(Mathf.Max(0, v.x), Mathf.Max(0, v.y), Mathf.Max(0, v.z), Mathf.Max(0, v.w));
        ...
        public static PolylinePoint WeightedSum(Vector4 w, PolylinePoint a, PolylinePoint b, PolylinePoint c,
            PolylinePoint d) => w.x * a + w.y * b + w.z * c + w.w * d;
        ...
        public static Vector2 Lerp(Rect r, Vector2 t) =>
            new Vector2(Mathf.Lerp(r.xMin, r.xMax, t.x), Mathf.Lerp(r.yMin, r.yMax, t.y));

        [MethodImpl(INLINE)]
        static Vector2 InverseLerp(Vector2 a, Vector2 b, Vector2 v) => (v - a) / (b - a);

        [MethodImpl(INLINE)]
        public static Vector2 InverseLerp(Rect r, Vector2 pt) => new Vector2(Mathf.InverseLerp(r.xMin, r.xMax, pt.x),
            Mathf.InverseLerp(r.yMin, r.yMax, pt.y));

        [MethodImpl(INLINE)]
        static Vector2 Remap(Vector2 iMin, Vector2 iMax, Vector2 oMin, Vector2 oMax, Vector2 value) =>
        ...
        public static float GetApproximateAngularCurveSumDegrees(Vector3 a, Vector3 b, Vector3 c, Vector3 d,
            int vertCount)
        {
            float angSum = 0f;
        ...
        public static Matrix4x4 AffineMtxMul(Matrix4x4 lhs, Matrix4x4 rhs)
        {
            ...
            m.m03 = (float)((double)lhs.m00 * rhs.m03 + lhs.m01 * (double)rhs.m13 + (double)lhs.m02 * rhs.m23 +
                            lhs.m03);
            ...
            m.m13 = (float)((double)lhs.m10 * rhs.m03 + lhs.m11 * (double)rhs.m13 + (double)lhs.m12 * rhs.m23 +
                            lhs.m13);
            ...
            m.m23 = (float)((double)lhs.m20 * rhs.m03 + lhs.m21 * (double)rhs.m13 + (double)lhs.m22 * rhs.m23 +
                            lhs.m23);
            m.m30 = 0;
            m.m31 = 0;
            m.m32 = 0;
