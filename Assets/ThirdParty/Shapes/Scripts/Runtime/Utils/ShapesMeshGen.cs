...
namespace Shapes
{
    public static class ShapesMeshGen
    {
        ...
        static readonly ExpandoList<Vector4>
            meshUv0 = new ExpandoList<Vector4>(); // UVs for masking. z contains endpoint status, w is thickness
            ...
            bool flattenZ, bool useColors)
        {
            ...
            for (int i = 0; i < pointCount; i++)
            {
                ...
                void SetUv0(ExpandoList<Vector4> uvArr, float uvEndpointVal, float pathThicc, int id, float x,
            ...
            float radius, float radiusInner, float angRadiansStart, float angRadiansEnd)
        {
            ...
            float apothemOuter = Mathf.Cos(0.5f * Mathf.Abs(gizmoAngEnd - gizmoAngStart) / segmentCount) *
